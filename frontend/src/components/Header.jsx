...
const Header = () => {
   ...
   const logout = () => {
    ...
    useEffect(()=>{
      const handleScroll = () => {
      if (window.scrollY >0){
         //close the menu if opended when scrolling occurs
         if (menuOpened){
            setMenuOpened(false)
         }
      }
  ...
  return (
    <header className='fixed top-0 w-full left-0 right-0 z-50'>
       <div className={`${active ? 'bg-white py-2.5':' bg-primary py-3'} max-padd-container 
         flexBetween border-b border-slate-900/10 rounded transition-all duration-300`}>
         {/*Logo*/}
         ...
         <div className='flex-1 flex items-center justify-end gap-x-3 sm:gap-x-10'>
            <HiMenuAlt1 onClick={toggleMenu} className='text-2xl xl:hidden cursor-pointer'/>
            <Link to ={'/cart'} className ='flex relative'>
                  <RiShoppingBag2Fill className='text-[33px]  bg-secondary text-primary p-1.5 rounded-full'/>
                  <span className='bg-primary ring-1 ring-slate-900/5 medium-14 absolute
                   left-5 -top-2.5 flexCenter w-5 h-5 rounded-full shadow-md'>{getCartCount()}</span>      
            </Link>

            <div className='relative group'>
               <div >
                  {token ? (
                     <TbUserCircle className='text-3xl cursor-pointer' />
                  ) : (
               ...
               {token && (
                  <ul className='bg-white p-1 w-32 ring-1 ring-slate-900/5 rounded absolute 
                      right-0 top-7 hidden group-hover:flex flex-col regular-14 shadow-md'>
